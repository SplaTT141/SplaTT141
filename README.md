# Hi there ✌️ I'm Ernest 👾

**`JUNIOR FULL STACK DEVELOPER`**

## About Me 🌟

- 💻 Full-Stack Developer
- 🤓 Constantly improving my skills
- 🌋 Learning by building projects and experimenting
- 💪 Enjoying challenges
- 🎯 Focused on continuous growth as a developer
- 🚀 Open to new job opportunities

## Tech Stack ⚒️

<img src="https://cdn.simpleicons.org/html5/E34F26" height="40"/> <img src="https://cdn.simpleicons.org/css/663399" height="40"/> <img src="https://cdn.simpleicons.org/javascript/F7DF1E" height="40"/> <img src="https://cdn.simpleicons.org/react/61DAFB" height="40"/> <img src="https://cdn.simpleicons.org/node.js/5FA04E" height="40"/> <img src="https://cdn.simpleicons.org/mysql/4479A1" height="40"/> <img src="https://cdn.simpleicons.org/git/f05032" height="40"/> <img src="https://cdn.simpleicons.org/github/302F2F" height="40"/> <img src="https://cdn.simpleicons.org/npm/cb3837" height="40"/> <img src="https://cdn.simpleicons.org/insomnia/4000bf" height="40"/>

## Projects 💼

### Full-Stack Project – **Kirpykla**

- 🛠️ Tech: React, Node.js, Express, MySQL, Bootstrap
- 📝 Full-stack app with authentication and CRUD features
- 🔗 Repo: https://github.com/SplaTT141/kirpykla
- 🌐 Demo: https://kirpykla.netlify.app/

---

### Frontend Project – **Sofax**

- 🛠️ Tech: JavaScript, CSS
- 📝 Group frontend project developed by a team of 4 people
- 👤 My responsibilities:
  - Building UI components
  - Implementing responsive design
- 🌐 Live Demo: https://splatt141.github.io/sofax/
- 🔗 Repository: https://github.com/SplaTT141/sofax

---

### 🧩 Mini Projects

- 📌 **Kanban Task Board**

  Interactive Kanban app with drag & drop

  🔗 Repo: https://github.com/SplaTT141/kanban | 🌐 Demo: https://splatt141.github.io/kanban/

- 📌 **Clock App**

  Digital & analog clock with world time and Pomodoro timer

  🔗 Repo: https://github.com/SplaTT141/clock | 🌐 Demo: https://splatt141.github.io/clock/

- 📌 **Notes App**

  Notes app with editable notes and delete confirmation

  🔗 Repo: https://github.com/SplaTT141/todo-list | 🌐 Demo: https://splatt141.github.io/todo-list/

- 📌 **Image Search App**

  Image search app using the Unsplash API for dynamic image loading

  🔗 Repo: https://github.com/SplaTT141/picture-search | 🌐 Demo: https://splatt141.github.io/picture-search/

- 📌 **Random Image Fetcher**

  Random image fetcher with input validation

  🔗 Repo: https://github.com/SplaTT141/random-pic | 🌐 Demo: https://splatt141.github.io/random-pic/

## Now Working On 🚧

- 🛒 Full-Stack **Vanilla JavaScript E-Commerce SPA**
- 🛠️ Tech: JavaScript, Express.js, Node.js, MySQL

## Reach Me ✍️

- 📧 ernest.ivanov01@gmail.com
- 📬 linkedin.com/in/ernest-ivanov
